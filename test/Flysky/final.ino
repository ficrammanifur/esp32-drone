...
void scanI2C() {
  ...
  for (address = 1; address < 127; address++) {
    ...
    if (error == 0) {
      ...
      nDevices++;
      if (address == 0x68) Serial.println("  -> MPU6050 detected!");
    }
  }
  if (nDevices == 0) Serial.println("No I2C devices found. Check wiring/power!");
...
void handlePostConfig() {
  if (server.hasArg("plain")) {
    ...
    // Simple JSON parsing (cari "true")
    config.lora_enabled = (body.indexOf("\"lora\":true") > -1);
    config.gps_enabled = (body.indexOf("\"gps\":true") > -1);
...
void setup() {
  ...
  // I2C & MPU6050 (INIT DULU SEBELUM WiFi!)
  Wire.begin();
  Wire.setClock(400000);
  delay(100);  // Stabilkan I2C
  ...
  if (!mpu.begin()) {
    ...
    Serial.println("Re-upload after fix wiring.");
    while (1) delay(10);
  }
  ...
  // WiFi AP (setelah I2C stabil)
  WiFi.mode(WIFI_AP);
  WiFi.softAP(ap_ssid, ap_password);
  delay(500);  // Tunggu AP stabil
  Serial.println("\n========================================");
  Serial.println("WiFi AP Started: " + String(ap_ssid));
  Serial.println("Password: " + String(ap_password));
  Serial.println("IP Address: " + WiFi.softAPIP().toString());
  Serial.println("========================================");
  Serial.println("1. Disconnect dari WiFi rumah");
  Serial.println("2. Connect ke WiFi: FRISS");
  Serial.println("3. Buka browser: http://192.168.4.1");
  Serial.println("========================================\n");
  ...
  Serial.println("iBUS initialized on GPIO16 ✓");
  ...
  writeAllESC(1000);
  delay(2000);
  Serial.println("ESCs armed (1000us - OFF) ✓");

  // Optional modules
  if (config.lora_enabled) initLoRa();
  if (config.gps_enabled) initGPS();

  Serial.println("=== SETUP COMPLETE === Arm via CH5 >1500 (NO PROP!)");
  Serial.println("Gerakkan stick untuk test channels. Monitor for iBUS data.");
}
...
void loop() {
  ...
  // Debug print iBUS (setiap 1s)
  if (now - prev_debug > 1000) {
    Serial.printf("DEBUG: CH1=%d CH2=%d CH3=%d CH4=%d CH5=%d | GPS Fix=%d Sat=%d\n",
                  raw_roll, raw_pitch, raw_throt, raw_yaw, raw_aux1, gps.location.isValid(), gps.satellites.value());
    prev_debug = now;
  }
  ...
  if (raw_throt < 1050 || raw_roll < 0 || raw_pitch < 0 || raw_yaw < 0) {
    writeAllESC(1000);
    Serial.println("FAILSAVE: Invalid iBUS data or low throttle!");
    delay(50);
    return;
  }
